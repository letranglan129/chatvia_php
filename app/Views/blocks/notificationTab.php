<div class="tab-pane fade h-100" id="notifications-tab" role="tabpanel">
    <div class="d-flex flex-column h-100">
        <!-- Tab Header -->
        <div class="tab-header d-flex align-items-center border-bottom">
            <ul class="d-flex justify-content-between align-items-center list-unstyled w-100 mx-4 mb-0">
                <!-- Title -->
                <li>
                    <h3 class="mb-0">Thông báo</h3>
                </li>
                <!-- Title -->

                <!-- Buttons -->
                <li>
                    <ul class="list-inline">
                        <!-- Menu Button -->
                        <li class="list-inline-item">
                            <button type="button" class="navigation-toggle btn btn-secondary btn-icon d-xl-none">
                                <i class="ri-menu-line"></i>
                            </button>
                        </li>
                        <!-- Menu Button -->
                    </ul>
                </li>
                <!-- Buttons -->
            </ul>
        </div>
        <!-- Tab Header -->

        <!-- Tab Content -->
        <div class="hide-scrollbar h-100">
            <div class="m-4 mt-0">
                <!-- Today -->
                <div class="notification-list notification-list-today">
                    <!-- Title -->
                    <div class="d-flex align-items-center mx-4 mt-3 mb-3">
                        <small class="text-muted me-auto">Mới</small>
                    </div>
                    <!-- Title -->

                    <!-- Card -->
                    <div class="list-wrap">

                    </div>
                    <!-- Card -->
                </div>
                <!-- Today -->

                <!-- Yesterday -->
                <div class="notification-list notification-list-yestoday">
                    <!-- Title -->
                    <div class="d-flex align-items-center mx-4 mb-3">
                        <small class="text-muted me-auto">Trước đó</small>
                    </div>
                    <!-- Title -->

                    <div class="list-wrap">

                    </div>
                </div>
                <!-- Yesterday -->
            </div>
        </div>
        <!-- Tab Content -->
    </div>
</div>